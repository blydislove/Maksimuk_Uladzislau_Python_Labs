...
1. Объединение записей CDS двух видов в единый файл.
2. Анализ и сортировка последовательностей по GC-составу.
3. Трансляция кодирующих областей (CDS) в белковые последовательности.

### Задания і шаги по выполнению
...
- Загрузка данных:
  ...
  - Выбрать 5 записей для каждого вида и сохранить их в формате GenBank.
- Объединение файлов:
  - Использовать ```SeqIO``` из Biopython для чтения отдельных файлов и записи всех записей в один файл.
- Проверка
  - Убедиться, что итоговый файл содержит не менее 10 CDS (по 5 для каждого вида) 

#### Задание 2
- Чтение файла:
  - Использовать ```SeqIO.parse()``` для загрузки данных из объединённого файла.
- Расчёт GC-состава:
...
#### Задание 3
- Извлечение CDS:
  - Для каждой записи в GenBank-файле найти все аннотации типа ```CDS``` (кодирующие последовательности).
- Учёт параметров трансляции:
  - Определить рамку считывания (```codon_start```, например, 1,2 или 3)
  - Учесть таблицу трансляции (```transl_table```, например, 1 для стандартного генетического кода).
- Трансляция в белковые последовательности:
  - Преобразовать нуклеотидную последовательность CDS в аминокислотную с помощью метода ```translate()```.
- Вывод результатов:
  - Для каждой CDS вывести:
    - Идентификатор записи и описание.
    - Локализацию CDS (начало, конец, цепь)
...
- Сортировка:
```python
sorted(sequences, key=lambda x: x[2]).
```
### Задание 3
- Библиотеки:
