...
- Объединение файлов:
  - Использовать ```python SeqIO``` из Biopython для чтения отдельных файлов и записи всех записей в один файл.
- Проверка
...
- Чтение файла:
  - Использовать ```python SeqIO.parse()``` для загрузки данных из объединённого файла.
- Расчёт GC-состава:
  - Для каждой последовательности вычислить GC-состав по формуле: ```math GC=(Amount(G+C)/(Sequence_length)``` 
